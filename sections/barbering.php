...
if (!isset($_SESSION['user']) || $_SESSION['user']['role'] !== 'barbering') {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['status'], $_POST['client_id'])) {
    ...
    $stmt->execute();
    header("Location: barbering.php");
    exit;
}

$result = $conn->query("SELECT * FROM clients WHERE section = 'Hair Barbering' ORDER BY created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Barbering Dashboard | The Breeze Spa</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
        ...
        .flat-card-header { border-bottom: 1px solid var(--border-color); padding-bottom: 15px; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center; }
...
<aside class="sidebar">
    ...
    <nav>
        <a href="barbering.php" class="menu-item active"><i class="fas fa-home" style="margin-right:12px;"></i> Dashboard</a>
    </nav>
...
<main class="main-content">
    ...
    <div class="flat-card">
        <div class="flat-card-header"><h3>Assigned Client Queue</h3></div>
        <div class="table-container">
            <table>
                ...
                <tbody id="worker-table-body">
                    ...
                    <?php endwhile; else: ?>
                    <tr><td colspan="5" style="text-align: center; color: var(--text-muted);">No assigned clients found.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    ...
    <script>
        ...
        async function pollQueueStatus() {
            try {
                ...
                if (clients.length === 0) {
                    tbody.innerHTML = `<tr><td colspan="5" style="text-align: center; color: var(--text-muted);">No assigned clients found.</td></tr>`;
                    clientIds.clear();
                    return;
                }
